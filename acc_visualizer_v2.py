"""
3D Accelerometer Visualizer v2 - Calibrated Hand Orientation

This version correctly handles different sensor orientations:

BOSCH (BHI260AP) - Baseline sensor:
- X-axis: Finger-to-shoulder direction (negative = toward fingers, positive = toward shoulder)
- Y-axis: Pinky-to-thumb direction (negative = toward thumb, positive = toward pinky)
- Z-axis: Through hand (negative = toward nails/back of hand, positive = toward palm/thigh)

NON-BOSCH (LIS3DSH) - Rotated 90° from Bosch:
- X-axis: Pinky-to-thumb direction (negative = toward thumb, positive = toward pinky)
- Y-axis: Finger-to-shoulder direction (negative = toward shoulder, positive = toward fingers)
- Z-axis: Through hand (negative = toward nails/back of hand, positive = toward palm/thigh)

Features:
- Realistic left hand model with watch/wearable
- Properly oriented based on actual hardware sensor mounting
- Automatic orientation correction for non_bosch sensors
- Motion trail and acceleration vector

Usage:
    python acc_visualizer_v2.py [csv_path]
"""
...
class LeftHandModel:
    ...
    - Y: Pinky-to-thumb direction (negative = toward thumb, positive = toward pinky)
...
def acc_to_rotation(acc_x, acc_y, acc_z, sensor_scale=4096.0):
    ...
    - Sensor Y: Thumb-to-pinky (negative = thumb, positive = pinky)
    ...
    gy = -sx  # Model Y (fingers) from -Sensor X (negative X = fingers)
    gz = -sz  # Model Z (back/nails up) from -Sensor Z (negative Z = nails)
...
class AccVisualizer:
    ...
    def _setup_3d(self):
        ...
        # Note: Sensor X=shoulder/finger, Y=thumb/pinky, Z=palm/nails
        ...
        # Gravity indicator (will point down)
...
def main():
    print("=" * 60)
    print("3D Accelerometer Visualizer v2 - Calibrated Orientation")
    print("=" * 60)
    print()
    print("Hand model: LEFT wrist with watch on upper side")
    print()
    print("Sensor axes (Bosch BHI260AP):")
    print("  X: Finger ↔ Shoulder (negative = toward fingers, positive = toward shoulder)")
    print("  Y: Thumb ↔ Pinky    (negative = toward thumb, positive = toward pinky)")
    print("  Z: Nails ↔ Palm     (negative = toward nails/back, positive = toward palm)")
    print()
    print("Non-Bosch sensor data is automatically corrected for 90° rotation difference")
    print()
