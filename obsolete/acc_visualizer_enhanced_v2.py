"""
Enhanced 3D Accelerometer Visualizer V2 - Fixed Gravity Frame

VERSION 2 CHANGES:
- Gravity axis (world Z) is now FIXED pointing downward
- Sensor X, Y, Z axes dynamically rotate with the hand orientation
- This makes it easier to see how the hand rotates relative to the fixed world frame
...
- Dynamic sensor axes that rotate with the hand
- Fixed world frame with gravity indicator

Usage:
    python acc_visualizer_enhanced_v2.py [csv_path]
...
class EnhancedVisualizerV2:
    """Enhanced visualizer V2 with fixed gravity frame and dynamic sensor axes."""
    ...
    def __init__(self, csv_path: str):
        ...
        self.show_sensor_axes = True  # New: show dynamic sensor axes
    ...
    def run(self):
        ...
        # Title
        self.fig.suptitle(f'Left Wrist Accelerometer V2 (Fixed Gravity Frame) - {self.csv_path.name}',
    ...
    def _setup_3d(self):
        """Setup 3D axes with fixed world frame."""
        # Start with larger limits for hand movement
        self.ax_3d.set_xlim(-3, 3)
        self.ax_3d.set_ylim(-3, 3)
        self.ax_3d.set_zlim(-3, 3)

        # Labels - WORLD frame (fixed)
        self.ax_3d.set_xlabel('World X', fontsize=9)
        self.ax_3d.set_ylabel('World Y', fontsize=9)
        self.ax_3d.set_zlabel('World Z (↑)', fontsize=9)

        self.ax_3d.set_title(f'{self.current_sensor.upper()} Sensor', fontsize=11)

        # FIXED view: gravity always points down (camera doesn't rotate)
        self.ax_3d.view_init(elev=20, azim=-60)

        # Draw reference floor (ground plane at Z=-2)
        xx, yy = np.meshgrid(np.linspace(-3, 3, 7), np.linspace(-3, 3, 7))
        self.floor = self.ax_3d.plot_surface(xx, yy, np.full_like(xx, -2.5), alpha=0.1, color='gray')

        # Fixed gravity arrow (always at origin pointing down)
        self.gravity_arrow = self.ax_3d.quiver(0, 0, 1, 0, 0, -1.5, color='purple', alpha=0.7,
                         arrow_length_ratio=0.15, linewidth=3, linestyle='--')
        self.gravity_text = self.ax_3d.text(0.2, 0.2, 0, '↓ Gravity', fontsize=10, color='purple', fontweight='bold')
    ...
    def _setup_info_panel(self):
        ...
        # Legend
        self.ax_info.text(0.5, 0.08, 'V2: Fixed gravity\nDynamic sensor axes', fontsize=8,
    ...
    def _setup_controls(self):
        ...
        # Options - Updated to include sensor axes
        ax_check = plt.axes([0.38, 0.01, 0.24, 0.035])
        ax_check.set_facecolor('#f5f5f5')
        self.check = CheckButtons(ax_check, ['Trail', 'Vector', 'Sensor Axes'], [True, True, True])
    ...
    def _on_reset(self, event):
        self.current_frame = 0
        self.trail_points = []
        self.hand_position = np.array([0.0, 0.0, 0.0])
    ...
    def _on_check(self, label):
        ...
        elif label == 'Sensor Axes':
            self.show_sensor_axes = not self.show_sensor_axes
        self._update_frame(self.current_frame)
    ...
    def _update_frame(self, idx):
        ...
        # Integrate velocity from acceleration over time (simplified - just for visualization)
        # For now, we'll use a simple approach: position based on recent accelerations
        if idx == 0:
            self.hand_position = np.array([0.0, 0.0, 0.0])
            self.velocity = np.array([0.0, 0.0, 0.0])
        else:
            # Get time delta
            dt = (self.timestamps[idx] - self.timestamps[idx-1]) / 1000.0  # Convert to seconds

            # Get acceleration in world frame (subtract gravity)
            acc_sensor = np.array([ax, ay, az]) / self.sensor_scale
            acc_model = np.array([acc_sensor[1], -acc_sensor[0], -acc_sensor[2]])
            acc_world = R @ acc_model

            # Subtract gravity (which is [0, 0, -1] in world frame)
            linear_acc = acc_world - np.array([0, 0, -1])

            # Simple integration (scale down for better visualization)
            scale_factor = 0.5  # Adjust this to control movement magnitude
            self.velocity += linear_acc * dt * scale_factor
            self.hand_position += self.velocity * dt

            # Apply damping to prevent drift
            self.velocity *= 0.98

        # Clear 3D (keep only floor and gravity arrow)
        while len(self.ax_3d.collections) > 1:
            self.ax_3d.collections[-1].remove()
        while len(self.ax_3d.lines) > 1:  # Keep gravity arrow
            self.ax_3d.lines[-1].remove()
        # Clear all text labels except the gravity text
        for txt in self.ax_3d.texts[1:]:  # Keep first text (gravity label)
            txt.remove()

        # Draw hand at current position
        meshes = self.hand.create_mesh(R)
        for name, verts, color in meshes:
            # Translate vertices by hand position
            verts_translated = verts + self.hand_position
            faces = self.hand._faces(verts_translated)
        ...
        # Watch/sensor position (wrist center) in world space
        wrist_local = R @ np.array([0, -0.5, 0.15])  # Watch position in hand frame
        wrist_pos = wrist_local + self.hand_position  # Translate to world position

        # Draw DYNAMIC sensor axes (rotate and move with hand)
        if self.show_sensor_axes:
            axis_len = 0.8

            # Transform sensor axes by rotation matrix
            sensor_x_dir = R @ np.array([0, -1, 0])  # Sensor X = -Model Y (toward fingers)
            sensor_y_dir = R @ np.array([1, 0, 0])   # Sensor Y = Model X (toward pinky)
            sensor_z_dir = R @ np.array([0, 0, -1])  # Sensor Z = -Model Z (toward nails)

            # Draw sensor axes from wrist position
            self.ax_3d.quiver(wrist_pos[0], wrist_pos[1], wrist_pos[2],
                            sensor_x_dir[0]*axis_len, sensor_x_dir[1]*axis_len, sensor_x_dir[2]*axis_len,
                            color='red', linewidth=2.5, arrow_length_ratio=0.15, alpha=0.9)
            self.ax_3d.quiver(wrist_pos[0], wrist_pos[1], wrist_pos[2],
                            sensor_y_dir[0]*axis_len, sensor_y_dir[1]*axis_len, sensor_y_dir[2]*axis_len,
                            color='green', linewidth=2.5, arrow_length_ratio=0.15, alpha=0.9)
            self.ax_3d.quiver(wrist_pos[0], wrist_pos[1], wrist_pos[2],
                            sensor_z_dir[0]*axis_len, sensor_z_dir[1]*axis_len, sensor_z_dir[2]*axis_len,
                            color='blue', linewidth=2.5, arrow_length_ratio=0.15, alpha=0.9)

            # Labels for sensor axes
            label_offset = 1.1
            self.ax_3d.text(wrist_pos[0] + sensor_x_dir[0]*axis_len*label_offset,
                          wrist_pos[1] + sensor_x_dir[1]*axis_len*label_offset,
                          wrist_pos[2] + sensor_x_dir[2]*axis_len*label_offset,
                          'X', color='red', fontsize=10, fontweight='bold')
            self.ax_3d.text(wrist_pos[0] + sensor_y_dir[0]*axis_len*label_offset,
                          wrist_pos[1] + sensor_y_dir[1]*axis_len*label_offset,
                          wrist_pos[2] + sensor_y_dir[2]*axis_len*label_offset,
                          'Y', color='green', fontsize=10, fontweight='bold')
            self.ax_3d.text(wrist_pos[0] + sensor_z_dir[0]*axis_len*label_offset,
                          wrist_pos[1] + sensor_z_dir[1]*axis_len*label_offset,
                          wrist_pos[2] + sensor_z_dir[2]*axis_len*label_offset,
        ...
        if self.show_trail and len(self.trail_points) > 1:
            ...
            colors = plt.cm.plasma(np.linspace(0.2, 1, len(trail) - 1))
        ...
        # Acceleration vector (in world frame)
        if self.show_acc_vector:
            # Transform acceleration from sensor frame to world frame
            acc_sensor = np.array([ax, ay, az]) / self.sensor_scale
            # Map sensor to model coordinates
            acc_model = np.array([acc_sensor[1], -acc_sensor[0], -acc_sensor[2]])
            # Rotate to world frame
            acc_world = R @ acc_model

            self.ax_3d.quiver(wrist_pos[0], wrist_pos[1], wrist_pos[2],
                             acc_world[0]*1.2, acc_world[1]*1.2, acc_world[2]*1.2,
                             color='orange', linewidth=3, arrow_length_ratio=0.15, alpha=0.9)

        # Dynamic camera following (optional - keeps hand in view)
        # Update axes limits to follow the hand
        margin = 2
        self.ax_3d.set_xlim(self.hand_position[0] - margin, self.hand_position[0] + margin)
        self.ax_3d.set_ylim(self.hand_position[1] - margin, self.hand_position[1] + margin)
        self.ax_3d.set_zlim(self.hand_position[2] - margin, self.hand_position[2] + margin)
...
def main():
    print("=" * 60)
    print("3D Accelerometer Visualizer V2 - Fixed Gravity Frame")
    print("=" * 60)
    print()
    print("VERSION 2 FEATURES:")
    print("  - Gravity axis (world Z) is FIXED pointing downward")
    print("  - Sensor X, Y, Z axes ROTATE with hand orientation")
    print("  - Better visualization of hand rotation in world space")
    print()
    ...
    print("  - Sensor Axes: Toggle dynamic sensor axes (NEW)")
    print("  - Drag on 3D view to rotate camera")
    print()

    viz = EnhancedVisualizerV2(csv_path)
    viz.run()
